...
INSERT INTO `Book` VALUES (10,'Olympic Champios', 'Pearson',13000);
...
SELECT * FROM `Book` WHERE `publisher` = '축구의 역사'

# 실습 4-21 [도서이름에 ‘축구’가 포함된 출판사를 조회하시오.]
SELECT `publisher` FROM `Book` WHERE `publisher` LIKE '축구%';
...
SELECT * FROM `Book` order BY `price` DESC ,`bookid` ASC;
...
# 실습 4-33 [가격이 8,000원 이상인 도서를 구매한 고객에 대하여 고객별 주문도서의 총 수량을 조
회하시오. 단, 두 권 이상 구매한 고객만 조회할 것]
...
FROM `Customer` AS a
JOIN `Order` AS b ON a.custId = b.custId GROUP BY a.custId order BY `name`;
...
# 실습 4-44 [ook 테이블에 새로운 도서 ‘스포츠 의학’을 삽입하시오]
...
# 실습 4-46
DELETE FROM `Customer` WHERE `custId` = 5;bookstore